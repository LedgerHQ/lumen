...
Set `multiline` to wrap a long address over several lines instead of truncating it. Prefix, QR scanner, clear button, helper text, and counter behave exactly as they do on a single-line field.
...
<Canvas of={AddressInputStories.WithMultiline} />

Equal `minLines` and `maxLines` give a fixed-height box that scrolls instead of growing:

<Canvas of={AddressInputStories.WithMultilineFixedHeight} />
...
`multiline` wraps a long address over several lines, bounded by `minLines` and `maxLines`:

<Source of={AddressInputStories.WithMultiline} />

Equal `minLines` and `maxLines` give a fixed-height box that scrolls once the ceiling is reached.

<Source of={AddressInputStories.WithMultilineFixedHeight} />

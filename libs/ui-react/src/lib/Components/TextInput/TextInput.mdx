...
- `status`: Set to `'error'` or `'success'` for border, label, and helper styling; omit for a neutral hint. Setting `status='error'` automatically sets `aria-invalid={true}` on the input for screen readers.

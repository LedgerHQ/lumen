...
MediaBanner is a promotional banner component that displays a background image alongside a title and description. It is designed for marketing or informational content and supports an optional click action and an optional close button for dismissibility.

## Anatomy

<Canvas of={MediaBannerStories.Base} />

- **MediaBanner**: Root `<button>` with surface background, rounded corners, and a fixed height.
...
Pass `onClick` to make the banner interactive. Hover and active styles are only applied when `onClick` is provided.

<Canvas of={MediaBannerStories.Clickable} />

### Disabled

When `disabled` is `true`, the banner becomes muted. Hover and active states are suppressed and clicks are blocked.

<Canvas of={MediaBannerStories.DisabledState} />
...
- The root renders as a `<button>`, which is keyboard-activatable and screen-reader friendly out of the box
- When `disabled`, the native `disabled` attribute blocks all clicks and keyboard activation
- Hover and active styles are suppressed when no `onClick` is provided, so non-interactive banners don't mislead users
...
Pass `onClick` to make the banner navigate or trigger an action:
...
>
  <MediaBannerTitle>Start Staking</MediaBannerTitle>
  <MediaBannerDescription>
    Earn rewards on your crypto assets.
  </MediaBannerDescription>
</MediaBanner>
```

...
```tsx
// Use render prop with SelectButtonTrigger for compact triggers
<SelectTrigger
  render={(renderProps) => (
    <SelectButtonTrigger {...renderProps} label='Filter' />
  )}
/>

// Use render prop for fully custom triggers
<SelectTrigger
  render={({ selectedValue, selectedContent }) => (
    <MyCustomTrigger selected={!!selectedValue}>
      {selectedContent}
    </MyCustomTrigger>
  )}
/>
...
```tsx
// Don't combine render with children or label — render takes precedence and the others are ignored
<SelectTrigger
  label='Ignored label'
  render={(renderProps) => (
    <SelectButtonTrigger {...renderProps} label='Filter' />

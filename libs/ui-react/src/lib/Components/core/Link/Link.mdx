...
`@radix-ui/react-slot` ships with `@ledgerhq/lumen-ui-react` and enables flexible composition patterns, allowing the Link to merge its props with child elements when needed.

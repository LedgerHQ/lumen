...
Set `multiline` to let the field grow with its content instead of scrolling sideways. Everything else is unchanged: floating label, clear button, helper text, and counter behave exactly as they do on a single-line field.
...
<Canvas of={TextInputStories.WithMultiline} />
...
> **Note:** The scroll indicator inside a clamped field is drawn by the platform, so unlike the web component there is no `scrollbarWidth` prop to configure it.
...
`multiline` lets the field grow with its content, bounded by `minLines` and `maxLines`:

<Source of={TextInputStories.WithMultiline} />

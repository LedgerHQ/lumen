...
describe('Snackbar', () => {
  describe('Rendering', () => {
    it('should render the title', () => {
      render(<Snackbar title='Payment done' />);
      expect(screen.getByText('Payment done')).toBeInTheDocument();
    });

    it('should truncate the title on a single line', () => {
      render(<Snackbar title='A very long snackbar title' />);
      expect(screen.getByText('A very long snackbar title')).toHaveClass(
        'truncate',
      );
    });

    it('should not render a leading icon for the info appearance', () => {
      const { container } = render(<Snackbar title='Info' appearance='info' />);
    ...
    it('should render a status icon for non-info appearances', () => {
      const { container } = render(
        <Snackbar title='Saved' appearance='success' />,
      );
    ...
    it('should render a spinner while loading', () => {
      render(<Snackbar title='Uploading' loading />);
  ...
  describe('Accessibility', () => {
    it('should use role="status" for info and success', () => {
      const { rerender } = render(<Snackbar title='Info' appearance='info' />);
      expect(screen.getByRole('status')).toBeInTheDocument();

      rerender(<Snackbar title='Saved' appearance='success' />);
      expect(screen.getByRole('status')).toBeInTheDocument();
    });

    it('should use role="alert" for warning and error', () => {
      const { rerender } = render(
        <Snackbar title='Careful' appearance='warning' />,
      );
      expect(screen.getByRole('alert')).toBeInTheDocument();

      rerender(<Snackbar title='Failed' appearance='error' />);
      expect(screen.getByRole('alert')).toBeInTheDocument();
    });
  });
  ...
  describe('Interactions', () => {
    it('should render the close button and fire onClose', () => {
      const onClose = vi.fn();
      render(<Snackbar title='Dismiss me' onClose={onClose} />);

      const closeButton = screen.getByRole('button', {
        name: 'components.snackbar.closeAriaLabel',
      });
    ...
    it('should not render a close button without onClose', () => {
      render(<Snackbar title='No close' />);
  ...
  describe('Styling', () => {
    it('should apply a custom className', () => {
      const { container } = render(
        <Snackbar title='Styled' className='mb-16' />,
      );
    ...
    it('should forward ref', () => {
      const ref = vi.fn();
      render(<Snackbar title='Ref' ref={ref} />);

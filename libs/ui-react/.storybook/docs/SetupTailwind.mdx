...
- **Theme System**: Dynamic theming with support for [light/dark modes](#-dark-mode-support) and [different brand themes](/docs/react-colors--docs)
- **Design Token Integration**: Direct mapping of design tokens to Tailwind utilities. [Explore foundations →](/docs/react-colors--docs)
- **Component Patterns**: Common patterns and best practices for component styling. [View components →](/docs/react-button--docs)
...
### 2. Register Lumen as a Tailwind source

Tailwind only emits a utility if it has seen the class name in a file it scans,
and it skips `node_modules` by default. Our components ship with their class
names compiled into the published JS, so your Tailwind build has to scan them.

Import our stylesheet next to Tailwind in your main CSS file — that's all:
...
If you use the charts package, add its stylesheet too:

```css
@import '@ledgerhq/lumen-ui-react-visualization/tailwind.css';
```



This is the legacy path. It works, but it pins your build to our internal `dist`
layout, which may change in any release — and when it does, the glob stops
matching and your utilities disappear with no build failure. Moving to Tailwind v4
and the import above is the durable fix.

### 3. VSCode Extensions and Setup
...
2. **ESLint Plugin for Tailwind** ([eslint-plugin-better-tailwindcss](https://github.com/schoero/eslint-plugin-better-tailwindcss))
   ...
   - Install with:
     ...
     ```
   - Configure ESLint to lint classes inside `cn` and `cva`:
     ```javascript
     callees: [
       ['cn', [{ match: 'strings' }]],
       [
         ...
         [
           { match: 'strings' },
           { match: 'objectValues', pathPattern: '^variants.*$' },
           {
             match: 'objectValues',
...
Our spacing scale uses a 1:1 pixel mapping, where each number directly corresponds to its pixel value. For example, m-4 equals 4px of margin. [Learn more about our spacing system](/docs/react-spacing--docs).
...
Our color system uses semantic tokens. [Explore our color system](/docs/react-colors--docs).
[background colors](/story/react-colors--background).
[text colors](/story/react-colors--text).
[border colors](/story/react-colors--border).
[Crypto colors](/story/react-colors--crypto).
[Discover colors](/story/react-colors--discover).
...
Pre-configured typography classes that handle font family, size, weight, line height, and letter spacing. [View typography guidelines](/story/react-typography--typography).
...
[See gradient examples](/story/react-gradients--gradients).
...
[View shadow system](/story/react-shadows--shadows).
...
[Explore border radius tokens](/docs/react-borderradius--docs).
...
## 🔧 Edge Case: Tokens Outside Tailwind (`cssVar`)

For contexts where class names don't work (SVG attributes, canvas, third-party libs), use the type-safe `cssVar()` utility:
...
Typos and invalid tokens are caught at compile time. Web-only — React Native should use `useTheme()` instead.
...
- [Button component](?path=/docs/react-button--docs) for usage examples
- [Foundations](/docs/react-colors--docs) to explore design tokens
- [Components](/docs/react-button--docs) to see all available components

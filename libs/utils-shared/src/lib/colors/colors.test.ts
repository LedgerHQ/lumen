...
import { AVATAR_COLOR_KEYS, resolveAvatarColor } from './colors.js';

describe('resolveAvatarColor', () => {
  it('should return a key from the palette', () => {
    expect(AVATAR_COLOR_KEYS).toContain(resolveAvatarColor('user-123'));
  });
  ...
  it('should handle an empty identifier', () => {
    expect(AVATAR_COLOR_KEYS).toContain(resolveAvatarColor(''));
  });

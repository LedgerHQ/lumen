...
`@radix-ui/react-slot` ships with `@ledgerhq/lumen-ui-react` and enables the `asChild` composition pattern, allowing the Button to merge its props with child elements when needed.

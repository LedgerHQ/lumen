---
name: code-connect
description: Creates and maintains Figma Code Connect files that map Figma components to code — `.figma.ts` template files (MCP-based) and parser-based `.figma.tsx` files. Use when the user mentions Code Connect, Figma component mapping, design-to-code translation, or asks to create/update .figma.ts or .figma.tsx files.
paths: "**/*.figma.tsx"
...
> **Note:** The bulk of this skill covers `.figma.ts` **template files** (MCP-based). For parser-based `.figma.tsx` files (using `figma.connect()`, published via CLI), see [Authoring parser-based `.figma.tsx` files](#authoring-parser-based-figmatsx-files) at the end.
...
# Authoring parser-based `.figma.tsx` files

These conventions apply to parser-based Code Connect files (`figma.connect()`, published via CLI) — as opposed to the `.figma.ts` template files above.

## Never put logic in `example`

The `example` function must be a pure, flat render of props. All conditional logic, branching, and value derivation belongs in `props`.
...
## Nest `figma.enum` / `figma.boolean` to encode multi-axis logic

When a prop's JSX depends on more than one Figma property, nest the mapping calls instead of branching in `example`.

```tsx
// ✅ GOOD — spot size resolved entirely in props
...
## Use a separate numeric prop when a size drives a third-party component

When an icon library component (e.g. `CryptoIcon`) needs a numeric `size`, derive it as a separate prop and use it inline — do not use conditional rendering.

```tsx
// ✅ GOOD — numeric size prop used directly in example
tagSize: figma.enum('size', { md: 16, sm: 12 }),

example: (props) => (
  <MediaTag leadingContent={<CryptoIcon size={props.tagSize} />} />
)
```

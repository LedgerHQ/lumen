...
Links render as text, so they can be nested inside a sentence and flow with it without affecting the line height:

<Canvas of={LinkStories.InTextUsage} />
...
The Link component is built on React Native's `Text`, so it can be nested inside a sentence without breaking the line height:

```tsx
<Text typography='body2' lx={{ color: 'base' }}>
  By continuing, you agree to our{' '}
  <Link size='sm' href='https://example.com/terms'>
    Terms & Conditions
  </Link>
  .
</Text>
...
The Link component accepts all `Text` props, e.g. press callbacks or line clamping:

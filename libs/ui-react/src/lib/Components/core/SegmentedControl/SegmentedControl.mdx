...
`tabLayout` controls how segment widths are calculated:

- **`fixed`** (default) — all segments share equal width and fill the container. Text truncates if needed.
- **`fit`** — each segment sizes to its content. The control is as wide as its content.
- **`fit-controls`** — same as `fit` but adds left/right arrow buttons when the control overflows its container.

...
Pass `className="flex-none"` to `NavBarTitle` so it does not expand and push the description away.
...
`NavBarLeading` owns the `flex-1` stretching, so pass `className="flex-none"` to `NavBarTitle` inside it:

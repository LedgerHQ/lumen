...
<Meta title='Core/SideBar/Docs' of={SideBarStories} />
...
> **Note:** The sidebar has fixed widths - `72px` when collapsed (icon-only) and `208px` when expanded (with labels).
...
import { Home, HomeFill, Wallet, WalletFill, SettingsAlt, SettingsAlt2 } from '@ledgerhq/lumen-ui-react/symbols';
  ...
  return (
    <SideBar active={active} onActiveChange={setActive}>
      <SideBarLeading>
        ...
        <SideBarItem value="wallet" icon={Wallet} activeIcon={WalletFill} label="Wallet" />
      </SideBarLeading>
      <SideBarTrailing>
  ...
  return (
    <SideBar active={getActiveValue(pathname)}>
      <SideBarLeading>
        ...
        <SideBarItem value="/accounts" icon={Wallet} activeIcon={WalletFill} label="Accounts" />
      </SideBarLeading>
    </SideBar>
  );
}
...
<DoVsDontRow>
  <DoBlockItem
    ...
    description='Wrap items in SideBarLeading/SideBarTrailing. Place the collapse toggle in SideBarTrailing.'
  >

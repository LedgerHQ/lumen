...
Equal `minLines` and `maxLines` give a fixed-height box. `scrollbarWidth='none'` hides the scroll
affordance once the ceiling is reached — the field still scrolls:

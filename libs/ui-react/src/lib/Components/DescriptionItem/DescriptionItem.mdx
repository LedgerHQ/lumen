...
The label and the trailing value each truncate after one line. When both sides have long content and horizontal space is constrained, the `priority` prop controls which side is protected from shrinking and truncation.

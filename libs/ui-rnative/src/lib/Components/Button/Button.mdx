...
> Note: For `icon-only` buttons, use the [IconButton](https://ldls-react-native.vercel.app/?path=/docs/core-iconbutton--docs) component instead. It's specifically designed for icon-only interactions with proper accessibility support.

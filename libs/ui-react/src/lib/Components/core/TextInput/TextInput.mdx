...
Set `multiline` to render a `textarea` that auto-grows with its content. `minLines` sets the
starting height and `maxLines` the ceiling — past it the field scrolls internally rather than
growing. Setting them to the same value gives a fixed box. The field never offers a resize handle.

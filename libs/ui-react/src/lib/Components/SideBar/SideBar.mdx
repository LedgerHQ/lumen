...
Labels can be strings or React nodes. Use React nodes to add Tags or other elements. When using custom label content, provide a `tooltipLabel` for the collapsed state.
...
Use React nodes for labels when you need to add Tags or other elements. Provide `tooltipLabel` for collapsed mode.

```tsx
import { Tag } from '@ledgerhq/lumen-ui-react';
import { Bell, BellFill } from '@ledgerhq/lumen-ui-react/symbols';
...
Use different icon variants (e.g., filled vs outlined) for visual feedback. When `activeIcon` is not provided, it falls back to `icon`.
...
<DoVsDontRow>
  <DoBlockItem
    title='Provide a distinct `activeIcon` when available'
...
<DoVsDontRow>
  <DoBlockItem
    title='Provide `tooltipLabel` for custom labels'
    description='When using a React node label, provide a string tooltip for the collapsed state.'
  >
...
```
  ...
  <DontBlockItem
    title="Don't forget `tooltipLabel` with custom labels"
    description='Without it, the tooltip may not display usable text in collapsed state.'
  >

...
Tile components are compact, card-like interactive elements designed for horizontal lists, carousels, or grids. They use a flexible composite pattern that allows you to compose tiles with icons, text content, tags, and custom elements in a vertical layout.

> View in [Figma](https://www.figma.com/design/JxaLVMTWirCpU0rsbZ30k7/2.-Components-Library?node-id=5783-1328).

## Anatomy

- **Tile**: Root container; direct children are laid out vertically (e.g. icon, TileContent)
...
- **Hover / Pressed**: Darker background when hovered or pressed
...
Tile components support various configurations including different icons, with/without description, and with/without trailing content (tags).
...
<div className='flex overflow-x-auto'>
  {items.map((item) => (
    <Tile key={item.id} className='w-128 shrink-0'>
      <Spot appearance='icon' icon={item.icon} />
      <TileContent>

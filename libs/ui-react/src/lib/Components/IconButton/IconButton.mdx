...
Use the `onTooltipShow` callback to react when the tooltip becomes visible. This is useful for analytics tracking, such as logging tooltip impressions to measure feature discoverability.
...
<IconButton
  ...
  icon={Settings}
  tooltip
  onTooltipShow={() => analytics.track('settings_tooltip_shown')}
/>
```

...
> View in [Figma](https://www.figma.com/design/JxaLVMTWirCpU0rsbZ30k7/2.-Components-Library?node-id=17523-37413&m=dev).
...
Use the `decimalSeparator` prop to control how decimals are **displayed** (`'.'` by default, or `','` for locales such as fr-FR). Typing always accepts both `,` and `.`, so the input stays compatible with any locale keyboard — only the displayed (and emitted) separator changes.

<Canvas of={AmountInputStories.DecimalSeparator} />
...
By default decimals are displayed with a dot. Pass `decimalSeparator=","` to display them with a comma (e.g. for fr-FR). Input parsing always accepts both `,` and `.`, so this only affects the displayed value and the value passed to `onChange`.

```tsx
// Displays "1 234,5" and emits "1 234,5" through onChange
<AmountInput
  value={value}
  onChange={handleChange}
  currencyText='€'

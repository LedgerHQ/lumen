...
<Meta title='Core/DataTable' of={DataTableStories} />
...
DataTable is a high-level abstraction built on top of [TanStack Table](https://tanstack.com/table/latest/docs/introduction) and the Lumen [Table](?path=/docs/react-table--docs) primitives.
...
> If you need full control over the table markup and layout, see [Table](?path=/docs/react-table--docs) for the underlying UI primitives.
...
Pass `getRowProps` to `DataTableRoot` to add native attributes, styles, classes, or refs to individual data rows. The resolver receives the full TanStack `Row` object. Lumen continues to own row click behavior and accessibility roles.

<Canvas of={DataTableStories.WithRowProps} />
...
By default, sorting, filtering, and pagination are processed client-side in the browser. Use `state.columnFilters` and `onColumnFiltersChange` to drive column-specific filters from external UI controls such as a `Select` — TanStack applies them automatically against the full dataset:
...
// Drive from any external control, e.g. a Select:
// setColumnFilters([{ id: 'category', value: 'DeFi' }]);
// Clear with: setColumnFilters([])
```

To delegate processing to your backend instead, pass the manual flags and wire the state callbacks to your data source. Set `manualFiltering: true` to tell TanStack not to filter the data itself — your data source is responsible for returning the already-filtered rows:
...
const data = useMyApi({ sorting, columnFilters, globalFilter });
...
Use `getRowProps` when each data row needs attributes derived from its original item.

<Source of={DataTableStories.WithRowProps} />

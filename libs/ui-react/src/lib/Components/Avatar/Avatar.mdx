...
The Avatar component displays a circular user profile image with automatic fallback to a User icon when the image is unavailable or fails to load. It supports an optional notification indicator and four size variants.
...
To show a status or alert dot, wrap the Avatar in a `DotIndicator` and derive the dot props from the avatar size with the `getDotConfig` helper:

```tsx
import { Avatar, DotIndicator, getDotConfig } from '@ledgerhq/lumen-ui-react';

<DotIndicator {...getDotConfig('avatar', 'md')} aria-label="New notifications">
...
> When the dot conveys state to the user, pass an `aria-label` to `DotIndicator`. Without one the dot is `aria-hidden` and won't be announced by assistive technology.
...
Wrap the Avatar in a `DotIndicator` to show a status or alert dot, using `getDotConfig` to match the dot to the avatar size:

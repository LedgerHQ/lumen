...
<Meta title='Core/Table' of={TableStories} />
...
> If you want built-in sorting, filtering, pagination, and more without wiring the UI yourself, see [DataTable](?path=/docs/react-datatable--docs) instead.
...
<Canvas of={TableStories.WithNetworkIconsAndActionBar} />

### Cell Content

Compose rich cell content with `TableCellItem` as the root. Place a leading
element (a `Spot`, icon or crypto-icon) directly inside it, followed by a
`TableCellContent` that stacks a `TableCellContentTitle` and a
`TableCellContentDescription`.

Set `align='end'` on `TableCell` (to position content at the right edge) and on `TableCellItem` (to right-align the content parts); `TableCellItem` alignment is propagated to the content sub-components via context.
For more complex compositions, use `TableCellContentRow` to lay out a
description alongside custom trailing content (e.g. a `Tag`).

<Canvas of={TableStories.WithComplexCellContent} />
...
Compose cell content with `TableCellItem` and its sub-components instead of a
single element. A leading element is a direct child; the text column is a
`TableCellContent`:
...
<TableCell align='end'>
  <TableCellItem align='end'>
    <TableCellContent>
      <TableCellContentTitle>$62,000.00</TableCellContentTitle>
      <TableCellContentDescription>+2.4%</TableCellContentDescription>
    </TableCellContent>
  </TableCellItem>
</TableCell>
```

Use `TableCellContentRow` for richer compositions, e.g. a description next to a
`Tag`:

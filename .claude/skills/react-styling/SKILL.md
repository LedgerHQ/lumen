...
- Source of truth: `libs/design-core/src/lib/utils/createCustomPlugin.ts` (see `createTypographyPlugin`).
...
- Use our design tokens from `createThemePlugin`:
  - Text colors: `text-base`, `text-on-accent`, `text-muted`, etc.
  - Background colors: `bg-base`, `bg-muted`, `bg-accent`, etc.
  - Border colors: `border-base`, `border-muted`, `border-accent`, etc.
...
**Small sizes**: 0, 1, 2, 4, 6, 8, 10, 12, 14, 16, 20, 24, 28, 32, 36, 40, 44, 48, 56, 64, 72, 80, 96
...
- Use template literals or array joining for dynamic class names

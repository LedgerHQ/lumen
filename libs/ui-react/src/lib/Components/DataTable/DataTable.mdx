...
Use `state.columnFilters` and `onColumnFiltersChange` on `useLumenDataTable` to drive column-specific filtering from external UI controls such as a `Select`.  
TanStack handles the filtering client-side automatically — no `manualFiltering` flag needed.

<Canvas of={DataTableStories.WithClientSideColumnFilter} />

## Column Filters (Server-Side)

Use `state.columnFilters` and `onColumnFiltersChange` on `useLumenDataTable` to drive column-specific filtering from external UI controls such as a `Select`.  
Set `manualFiltering: true` and wire the state callbacks to your API:

<Canvas of={DataTableStories.WithServerSideFilters} />
...
The story below shows a complete example with a search input and a category filter both wired to a simulated async backend. Notice the loading indicator while the "request" is in flight — this is the key visual difference from client-side filtering.

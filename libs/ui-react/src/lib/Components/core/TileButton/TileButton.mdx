...
Labels are automatically truncated to 2 lines maximum:
...
`@radix-ui/react-slot` ships with `@ledgerhq/lumen-ui-react` and enables flexible composition patterns, allowing the TileButton to merge its props with child elements when needed.

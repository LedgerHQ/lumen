...
`BottomSheetContent` is a non-scrollable content block that applies standard paddings. It detects whether it lives inside a scrollable parent and strips its own paddings when the parent already provides it.
...
`BottomSheetFooter` renders a sticky area at the bottom of the sheet. It must be placed inside a `BottomSheet` (context is required). On mount it signals its presence to the root context so sibling scrollables and `BottomSheetContent` automatically drop their bottom padding.

When placed inside a `BottomSheetView` or any scrollable wrapper, it detects this via context and removes its own horizontal padding because the parent already provides it.

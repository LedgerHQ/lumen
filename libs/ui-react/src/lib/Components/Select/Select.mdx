...
`SelectButtonTrigger` is a built-in preset that renders a compact button-style trigger. It shows a label when nothing is selected, and the selected item's text once a value is picked. It supports icons, crypto icons, all `ButtonTrigger` appearances, and you can also build a fully custom trigger from scratch using the `render` function directly.

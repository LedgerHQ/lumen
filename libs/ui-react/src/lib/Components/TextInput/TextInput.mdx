...
```
  ...
  <DoVsDontRow>
    <DoBlockItem
      title='Use either label or placeholder, not both'
      description='Choose one approach - use floating label for better UX, or placeholder as an alternative. They serve the same purpose'
    >

{/* prettier-ignore */}
```tsx
// Use floating label (recommended)
<TextInput 
  label='Email Address'
  value={value}
  onChange={(e) => setValue(e.target.value)} 
...
// OR use placeholder (alternative)
<TextInput 
...
```
    ...
    <DontBlockItem
      title="Don't combine label and placeholder"
      description='They will visually overlap and create a poor user experience. The floating label animates from inside the input field'
    >

...
> **Note**: The `DialogHeader` close button automatically uses `DialogClose` internally, so the dialog always closes when the close button is clicked. The `onClose` prop on `DialogHeader` is optional — when provided, it fires as a side-effect callback (useful for analytics or cleanup).

...
Tile components are compact, card-like interactive elements designed for horizontal lists, carousels, or grids. They use a flexible composite pattern (same as Card) that allows you to compose tiles with Spot and TileContent (TileTitle, TileDescription, TileTrailingContent) as direct children in a vertical layout.
...
- Spot and other custom elements can be placed as direct children of Tile
...
### Trailing content (Tags, etc.)

<DoVsDontRow>
  <DoBlockItem
    title='Use TileTrailingContent for Tags and supplementary content'
    description='Wrap Tags, labels, or other trailing content inside TileTrailingContent within TileContent for proper spacing.'
  >
...
```
  ...
  <DontBlockItem
    ...
    description='Place Tags and other supplementary content inside TileTrailingContent, not as direct children of TileContent.'
  >

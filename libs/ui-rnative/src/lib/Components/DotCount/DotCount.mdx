...
<CustomTabs>
  <Tab label="Overview">
    ...
    DotCount displays a numeric count inside a colored pill. It supports two sizes (`md` and `sm`), two appearances (`base` and `negative`), and can be pinned to the top-right of a child element.
    ...
    - **sm** - compact pill, ideal for tight layouts or smaller containers.
  ...
  <Tab label="Implementation">
    ...
    function MyComponent() {
      return <DotCount value={5} size="md" />;
    }
    ```
    ...
    ```tsx
    <DotCount value={150} max={50} size="md" />
    {/* Renders "50+" */}
    ```

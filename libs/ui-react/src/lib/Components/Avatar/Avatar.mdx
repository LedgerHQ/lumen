...
To show a status or alert dot, wrap the Avatar in a `DotIndicator` and derive the dot size from the avatar size with the `getDotSize` helper:

```tsx
import { Avatar, DotIndicator, getDotSize } from '@ledgerhq/lumen-ui-react';

<DotIndicator size={getDotSize('avatar', 'md')} aria-label="New notifications">
...
Wrap the Avatar in a `DotIndicator` to show a status or alert dot, using `getDotSize` to match the dot to the avatar size:

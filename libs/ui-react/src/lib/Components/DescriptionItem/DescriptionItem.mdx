...
- **DescriptionItemLabel**: Muted text label. Truncates after two lines.
...
The label truncates after two lines and the trailing value after one. When both sides have long content and horizontal space is constrained, the `priority` prop controls which side is protected from shrinking and truncation.

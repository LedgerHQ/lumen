...
To show a status or alert dot, wrap the Avatar in a `DotIndicator` and derive the dot size from the avatar size with the `getDotIconSize` helper:
...
Wrap the Avatar in a `DotIndicator` to show a status or alert dot, using `getDotIconSize` to match the dot to the avatar size:
...
```tsx
import { Avatar, Box, Text, DotIndicator, getDotIconSize } from '@ledgerhq/lumen-ui-rnative';

...
`SelectButtonTrigger` is a built-in preset that renders a compact button-style trigger. It shows a label when nothing is selected, and the selected item's text once a value is picked. It supports icons, crypto icons, all [`ButtonTrigger`](/docs/action-buttontrigger--docs) appearances, and you can also build a fully custom trigger from scratch using the `render` function directly.
...
`SelectButtonTrigger` is a ready-made preset for button-style triggers. It accepts all [`ButtonTrigger`](/docs/action-buttontrigger--docs) props (`appearance`, `size`, `icon`, `iconType`) plus a required `label`.
